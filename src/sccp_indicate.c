...
// 			iPbx.set_callstate(c, AST_STATE_RINGING);						/*!\todo thats not the right place to update pbx state */
		...
		case SCCP_CHANNELSTATE_CONNECTED:
			...
			if (c->rtp.audio.writeState == SCCP_RTP_STATUS_INACTIVE || c->previousChannelState == SCCP_CHANNELSTATE_HOLD || c->previousChannelState == SCCP_CHANNELSTATE_CALLTRANSFER || c->previousChannelState == SCCP_CHANNELSTATE_CALLCONFERENCE || c->previousChannelState == SCCP_CHANNELSTATE_OFFHOOK) {
				sccp_channel_openReceiveChannel(c);
			} else if (c->rtp.audio.writeState != SCCP_RTP_STATUS_INACTIVE) {
		...
		case SCCP_CHANNELSTATE_PROGRESS:								/* \todo SCCP_CHANNELSTATE_PROGRESS To be checked */
			...
			if (c->previousChannelState == SCCP_CHANNELSTATE_CONNECTED) {
				...
				if (c->rtp.audio.writeState == SCCP_RTP_STATUS_INACTIVE && d->earlyrtp <= SCCP_EARLYRTP_PROGRESS) {
					sccp_channel_openReceiveChannel(c);
				}
		...
		case SCCP_CHANNELSTATE_PROCEED:
			...
			if (c->rtp.audio.writeState == SCCP_RTP_STATUS_INACTIVE && d->earlyrtp <= SCCP_EARLYRTP_RINGOUT) {
				sccp_channel_openReceiveChannel(c);
			}
			break;
		...
		case SCCP_CHANNELSTATE_DIALING:
			...
			if (d->earlyrtp <= SCCP_EARLYRTP_DIALING && c->rtp.audio.writeState == SCCP_RTP_STATUS_INACTIVE) {
				sccp_channel_openReceiveChannel(c);
			}
			break;
