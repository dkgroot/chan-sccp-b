...
void sccp_channel_openReceiveChannel(constChannelPtr channel)
{
	...
	if (sccp_rtp_getPbxRtp(audiortp) == NULL) {
		...
		sccp_log((DEBUGCAT_RTP + DEBUGCAT_CHANNEL)) (VERBOSE_PREFIX_2 "%s: Open receive channel with format %s[%d], payload %d, echocancel: %d, passthrupartyid: %u, callid: %u\n", DEV_ID_LOG(d), codec2str(sccp_rtp_getWriteFormat(audiortp)), sccp_rtp_getWriteFormat(audiortp), sccp_rtp_getWriteFormat(audiortp), channel->line->echocancel, channel->passthrupartyid, channel->callid);
