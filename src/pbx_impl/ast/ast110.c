...
static void sccp_sync_capabilities_with_peer(sccp_channel_t *c, PBX_CHANNEL_TYPE * ast)
{
	...
	for (; (remotePeer = ast_channel_iterator_next(iterator)); ast_channel_unref(remotePeer)) {
		if (pbx_find_channel_by_linkid(remotePeer, ast->linkedid)) {
...
static boolean_t sccp_wrapper_asterisk110_getPickupExtension(const sccp_channel_t * channel, char **extension)
{
	boolean_t res = FALSE;
...
static int sccp_wrapper_asterisk110_answer(PBX_CHANNEL_TYPE * astchan)
{
...
static void sccp_wrapper_asterisk110_setCalleridAni(const sccp_channel_t * channel, const char *number)
{
	if (number) {
