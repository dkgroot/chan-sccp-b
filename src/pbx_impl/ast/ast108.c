	...
	for (; (remotePeer = ast_channel_iterator_next(iterator)); ast_channel_unref(remotePeer)) {
		if (pbx_find_channel_by_linkid(remotePeer, ast->linkedid)) {
			...
			if (remoteSccpChannel) {
				...
				get_skinnyFormats(remotePeer->nativeformats, tmpCodecs, SKINNY_MAX_CAPABILITIES);
			}
		...
		case AST_CONTROL_RINGING:
			if (SKINNY_CALLTYPE_OUTBOUND == c->calltype) {
				...
				if (d->earlyrtp == SCCP_EARLYRTP_IMMEDIATE) {
					...
					//sccp_wrapper_asterisk18_setDialedNumber(c, c->dialedNumber);
				}
...
static int sccp_wrapper_asterisk18_answer(PBX_CHANNEL_TYPE * astchan)
{
